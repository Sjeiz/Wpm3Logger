...
Logging strategie:
Elke statuswijziging (state change) wordt direct gelogd naar zowel de seriële monitor als de web-logbuffer.
Elke 30 seconden wordt een gedetailleerde statusregel gelogd (periodieke statusupdate).
Warnings en errors worden direct gelogd naar beide outputs.
De web-log gebruikt een circulaire buffer (grootte gedefinieerd via LOG_BUFFER_SIZE), waarbij de nieuwste berichten vooraan staan en oude berichten automatisch worden overschreven.
Als een belangrijke logregel (zoals een state change of warning/error) uit de buffer dreigt te verdwijnen, wordt deze opnieuw gelogd zodat deze zichtbaar blijft.
De logica voor serieel en web is zoveel mogelijk samengevoegd, zodat dubbele code wordt voorkomen.
